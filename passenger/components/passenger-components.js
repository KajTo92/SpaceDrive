...
export function RideCard(ride, completed = false) {
  ...
  const confirmedStatuses = new Set(["confirmed", "driver_assigned", "driver_on_the_way", "driver_arrived", "passenger_onboard", "completed"]);
  const fullyConfirmed = confirmedStatuses.has(ride.status) && Boolean(ride.driver && ride.vehicle);
  const confirmation = fullyConfirmed ? `<span class="journey-confirmation journey-confirmation--confirmed">${icon("circle-check")} Confirmed</span>` : `<span class="journey-confirmation journey-confirmation--pending">${icon("clock-3")} Awaiting confirmation</span>`;
  return `<article class="ride-card ride-card--${fullyConfirmed ? "confirmed" : "pending"}"><a class="ride-card__main" href="${tripUrl(ride.id)}"><time><strong>${formatDate(ride.pickupDate, { day: "2-digit" })}</strong><span>${formatDate(ride.pickupDate, { month: "short" })}</span><small>${ride.pickupTime}</small></time><div class="ride-card__route">${routeMarkup}<small>${ride.vehicle?.brand || "Vehicle"} ${ride.vehicle?.model || "pending"}</small></div><div class="ride-card__meta">${ServiceTypeBadge(ride)}${confirmation}<strong>${money(ride)}</strong><small>${ride.driver?.shortName || "Driver pending"}</small></div></a>${completed ? `<div class="ride-card__actions"><button type="button" data-demo-action="Book again">Book again</button><button type="button" data-demo-action="Invoice">Invoice</button><a href="${tripUrl(ride.id)}">Details</a></div>` : ""}</article>`;
}
...
export function DriverCard(driver, expanded = false) {
  ...
  return `<article class="driver-card${expanded ? " driver-card--expanded" : ""}"><div class="driver-card__photo">${photo}</div><div class="driver-card__content"><span>Your chauffeur</span><h2>${driver.name}</h2><p>${driver.role || "Chauffeur"}</p><div class="driver-stats"><div>${icon("star")}<strong>5.0</strong><span>Rating</span></div><div><strong>100+</strong><span>Journeys</span></div></div><div class="driver-languages">${(driver.languages || []).map((language) => `<span>${language}</span>`).join("")}</div><div class="driver-actions"><button class="passenger-button" type="button" disabled title="Contact details are not connected">${icon("phone")} Call</button><button class="passenger-button" type="button" disabled title="Messaging is not connected">${icon("message-circle")} Message</button><button class="passenger-button" type="button" disabled title="WhatsApp is not connected">${icon("message-square")} WhatsApp</button></div></div></article>`;
}
