...
async function renderRideDetails() {
  ...
  const content = `<a class="driver-back-link" href="${driverUrl("rides/")}">${icon("arrow-left")} Back to rides</a><section class="driver-detail-hero"><div><span>${formatDate(ride.pickupDate, { weekday: "long", day: "numeric", month: "long" })} at ${ride.pickupTime}</span><h2>${ride.pickup.name}${icon("arrow-right")}${ride.destination.name}</h2></div><div class="driver-detail-hero__badges">${DriverServiceBadge(ride)}<span class="driver-status-badge driver-status-badge--${ride.status}" data-current-ride-status>${statusLabel(ride.status)}</span></div></section>${completion}<section class="driver-detail-status"><header><h2>Journey status</h2><strong data-current-ride-status-label>${statusLabel(ride.status)}</strong></header>${DriverRideAction(ride)} </section>${DriverTripMap(ride)}<section class="driver-detail-grid">${PassengerCard(ride)}${PassengerPreferencesCard(ride)}${FlightInfoCard(ride)}${LocationPermissionCard(permission.permission, permission.isTracking)}</section><section class="driver-journey-facts"><div><span>Pickup</span><strong>${ride.pickup.address}</strong></div><div><span>Destination</span><strong>${ride.destination.address}</strong></div><div><span>Vehicle</span><strong>${ride.vehicle ? `${ride.vehicle.brand} ${ride.vehicle.model}` : "Not assigned"}</strong></div><div><span>Notes</span><strong>${ride.notes || "No notes"}</strong></div></section>${QuickActions(ride)}${RideIssueModal(ride)}<section class="driver-danger-zone"><div><strong>Delete journey</strong><span>Permanently remove this journey and its updates.</span></div><button class="driver-secondary-button driver-secondary-button--danger" type="button" data-delete-journey>${icon("trash-2")} Delete journey</button></section>${DriverRideAction(ride, true)}`;
