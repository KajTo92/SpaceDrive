...
async function renderDashboard() {
  ...
  const content = `
    ...
    <section class="admin-metrics" aria-label="Today's operational metrics">
      ...
      ${metricCard("Drivers available", dashboard.metrics.driversAvailable, "users-round", "neutral", adminUrl("drivers/"))}
...
async function renderDispatch() {
  ...
  const content = `${PageHeading("Dispatch board", "One operating view for today's rides, chauffeurs and fleet assignments.")}<section class="dispatch-summary"><div><span>Unassigned rides</span><strong>${unassigned.length}</strong><small>${unassigned.length ? "Assignment required" : "Everything is assigned"}</small></div><div><span>Turnaround buffer</span><strong>${MIN_TURNAROUND_MINUTES} min</strong><small>Minimum between journeys</small></div><div><span>Operating window</span><strong>05:00-23:00</strong><small>${today.length} journeys today</small></div></section>
    <section class="dispatch-unassigned"><header><h2>Unassigned rides</h2><span>${unassigned.length}</span></header><div>${unassigned.length ? unassigned.map((ride) => `<a href="${adminUrl(`journeys/detail.html?id=${ride.id}`)}"><time>${escapeHtml(ride.pickupTime)}</time><span><strong>${escapeHtml(ride.pickup.name)} to ${escapeHtml(ride.destination.name)}</strong><small>${escapeHtml(serviceTypeLabel(ride))} · ${!ride.driverId ? "Driver required" : ""}${!ride.driverId && !ride.vehicleId ? ", " : ""}${!ride.vehicleId ? "Vehicle required" : ""}</small></span>${icon("arrow-right")}</a>`).join("") : EmptyState("Everything is assigned", "No unassigned rides remain for today.")}</div></section>
...
async function renderDrivers() {
  ...
  const cards = drivers.map((driver) => { const upcoming = journeys.filter((ride) => ride.driverId === driver.id && ride.pickupDate >= OPERATIONS_DATE && !["completed", "cancelled"].includes(ride.status)); return `<article class="driver-admin-card"><header><span class="driver-admin-photo">${driver.photo ? `<img src="${assetUrl(driver.photo)}" alt="Portrait of ${escapeHtml(driver.name)}">` : initials(driver.name)}</span><div><h2>${escapeHtml(driver.name)}</h2><p>${escapeHtml(driver.email)}</p></div>${StatusBadge(driver.availability)}</header><dl><div><dt>Phone</dt><dd>${escapeHtml(driver.phone)}</dd></div><div><dt>Languages</dt><dd>${escapeHtml(driver.languages.join(", "))}</dd></div><div><dt>Completed rides</dt><dd>${driver.completedTrips}</dd></div><div><dt>Upcoming rides</dt><dd>${upcoming.length}</dd></div></dl><footer><label class="admin-button driver-photo-upload">${icon("camera")} Upload photo<input type="file" accept="image/jpeg,image/png,image/webp" data-driver-photo="${driver.id}" hidden></label><button class="admin-button" type="button" data-driver-unavailable="${driver.id}">${icon("calendar-x")} Set unavailable</button></footer></article>`; }).join("");
